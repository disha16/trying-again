  ...
  try {
    ...
    res.json({
      ...
      search_providers: {
        ...
        SERPER_API_KEY:     !!(process.env.SERPER_API_KEY || process.env.serper_api_key),
...
app.post('/api/chart-sources', async (req, res) => {
  try {
    ...
    const cleaned = sources.map(s => {
      // Default kind is 'rss' (the only one that reliably works on Vercel).
      // Allowed kinds: 'rss' | 'html' | 'twitter' (legacy, kept for back-compat).
      const kind = ['rss', 'html', 'twitter'].includes(s.kind) ? s.kind : 'rss';
      return {
        name:       String(s.name || '').slice(0, 80),
        kind,
        url:        String(s.url || ''),
        chartFirst: s.chartFirst !== false,        // default true
        limit:      Math.min(5, Math.max(1, Number(s.limit) || 1)),
      };
